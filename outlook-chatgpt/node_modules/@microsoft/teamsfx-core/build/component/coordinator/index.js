...
class Coordinator {
    async create(context, inputs, actionContext) {
        ...
        if (scratch === question_1.ScratchOptionNo.id) {
        ...
        else if (scratch === question_1.ScratchOptionYes.id) {
            // create from new
            ...
            if (feature === constants_1.TabSPFxNewUIItem.id) {
                ...
                if (feature === constants_1.M365SsoLaunchPageOptionItem.id || feature === constants_1.M365SearchAppOptionItem.id) {
    ...
    async preProvisionForVS(ctx, inputs) {
        ...
        const templatePath = (_a = inputs["workflowFilePath"]) !== null && _a !== void 0 ? _a : path.join(ctx.projectPath, teamsfx_api_1.SettingsFolderName, process.env.TEAMSFX_ENV === "local" ? localWorkflowFileName : workflowFileName);
    ...
    async provision(ctx, inputs, actionContext) {
        ...
        for (const cycle of cycles) {
            ...
            if (unresolvedPlaceHolders.includes("AZURE_SUBSCRIPTION_ID")) {
                if (inputs["targetSubscriptionId"]) {
                    ...
                    const ensureRes = await provisionUtils_1.provisionUtils.ensureSubscription(ctx.azureAccountProvider, process.env.AZURE_SUBSCRIPTION_ID);
                    if (ensureRes.isErr())
                        return teamsfx_api_1.err(ensureRes.error);
            ...
            if (unresolvedPlaceHolders.includes("AZURE_RESOURCE_GROUP_NAME")) {
                ...
                if (inputRG && inputLocation) {
                    ...
                    const ensureRes = await provisionUtils_1.provisionUtils.ensureResourceGroup(ctx.azureAccountProvider, process.env.AZURE_SUBSCRIPTION_ID, process.env.AZURE_RESOURCE_GROUP_NAME, defaultRg);
                    if (ensureRes.isErr())
                        return teamsfx_api_1.err(ensureRes.error);
        ...
        if (targetResourceGroupInfo.createNewResourceGroup) {
            const createRgRes = await ResourceGroupHelper_1.resourceGroupHelper.createNewResourceGroup(targetResourceGroupInfo.name, ctx.azureAccountProvider, process.env.AZURE_SUBSCRIPTION_ID, targetResourceGroupInfo.location);
        ...
        if (azureSubInfo) {
            const url = tools_1.getResourceGroupInPortal(azureSubInfo.subscriptionId, azureSubInfo.tenantId, process.env.AZURE_RESOURCE_GROUP_NAME);
    ...
    async publish(ctx, inputs, actionContext) {
        const parser = new parser_1.YamlParser();
        const templatePath = path.join(ctx.projectPath, teamsfx_api_1.SettingsFolderName, process.env.TEAMSFX_ENV === "local" ? localWorkflowFileName : workflowFileName);
        const maybeProjectModel = await parser.parse(templatePath);
        if (maybeProjectModel.isErr()) {
            return teamsfx_api_1.err(maybeProjectModel.error);
        }
    ...
    async publishInDeveloperPortal(ctx, inputs, actionContext) {
        ...
        const updateRes = await appStudio_1.updateManifestV3ForPublish(ctx, inputs);
        if (updateRes.isErr()) {
