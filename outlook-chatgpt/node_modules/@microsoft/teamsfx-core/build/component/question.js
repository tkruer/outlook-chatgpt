...
async function getQuestionsForAddFeatureV3(ctx, inputs) {
    ...
    if (inputs.platform === teamsfx_api_1.Platform.CLI_HELP) {
        ...
        options.push(constants_2.WorkflowOptionItem);
...
exports.InitDebugProceedQuestion = {
    ...
    staticOptions: [exports.InitOptionYes, exports.InitOptionNo],
    default: exports.InitOptionYes.id,
};
exports.InitInfraProceedQuestion = {
    type: "singleSelect",
    name: "proceed",
    title: (inputs) => {
        const fileList = inputs["capability"] === exports.InitCapabilityBot.id
            ? "  teamsfx/\n    - app.yml\n    - .env.dev\n    - settings.json\n  infra/\n    botRegistration/\n      - azurebot.bicep\n      - readme.md\n    - azure.bicep\n    - azure.parameters.json\n"
            : inputs["spfx"] === exports.InitOptionYes.id
                ? "  teamsfx/\n    - app.yml\n    - .env.dev\n    - settings.json\n"
                : "  teamsfx/\n    - app.yml\n    - .env.dev\n    - settings.json\n  infra/\n    - azure.bicep\n    - azure.parameters.json\n";
        return localizeUtils_1.getLocalizedString("core.InitGenerateConfirm", fileList);
    },
...
function getQuestionsForInit(type, inputs) {
    ...
    group.addChild(new teamsfx_api_1.QTreeNode(exports.InitEditorQuestion));
    const capabilityNode = new teamsfx_api_1.QTreeNode(exports.InitCapabilityQuestion);
    group.addChild(capabilityNode);
    const SPFxNode = new teamsfx_api_1.QTreeNode(exports.InitIsSPFxQuestion);
    SPFxNode.condition = {
        validFunc: (input, inputs) => {
            if ((inputs === null || inputs === void 0 ? void 0 : inputs.editor) === exports.InitEditorVSCode.id && (inputs === null || inputs === void 0 ? void 0 : inputs.capability) === exports.InitCapabilityTab.id)
                return undefined;
    ...
    if (inputs.platform !== teamsfx_api_1.Platform.CLI_HELP) {
        group.addChild(new teamsfx_api_1.QTreeNode(type === "debug" ? exports.InitDebugProceedQuestion : exports.InitInfraProceedQuestion));
    }
    return teamsfx_api_1.ok(group);
}
exports.getQuestionsForInit = getQuestionsForInit;
async function getQuestionsForPublishInDeveloperPortal(inputs) {
    if (!inputs.projectPath) {
        return teamsfx_api_1.err(new error_1.ObjectIsUndefinedError("projectPath"));
    }
    const node = new teamsfx_api_1.QTreeNode({ type: "group" });
    let manifestDefaultPath = path_1.default.join(inputs.projectPath, "appPackage", "manifest.template.json");
    if (!(await fs_extra_1.default.pathExists(manifestDefaultPath))) {
        manifestDefaultPath = undefined;
    }
    node.addChild(new teamsfx_api_1.QTreeNode(manifestFileQuestion(manifestDefaultPath)));
    return teamsfx_api_1.ok(node);
}
exports.getQuestionsForPublishInDeveloperPortal = getQuestionsForPublishInDeveloperPortal;
const manifestFileQuestion = (defaultFile) => {
    return {
        type: "singleFile",
        name: question_2.CoreQuestionNames.ManifestPath,
        title: localizeUtils_1.getLocalizedString("core.question.manifestForPublishInDeveloperPortal.title"),
        placeholder: localizeUtils_1.getLocalizedString("core.question.manifestForPublishInDeveloperPortal.placeholder"),
        default: defaultFile,
    };
};
