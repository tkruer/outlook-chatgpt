...
const ProjectMigratorMWV3 = async (ctx, next) => {
    ...
    if (versionForMigration.state === versionMetadata_1.VersionState.upgradeable && projectMigrator_1.checkMethod(ctx)) {
...
async function checkVersionForMigration(ctx) {
    ...
    if (semver.gte(version, versionMetadata_1.MetadataV3.projectVersion)) {
        return {
            ...
            state: versionMetadata_1.VersionState.compatible,
            platform: platform,
        };
    }
        ...
        return {
            currentVersion: version,
            state: versionMetadata_1.VersionState.upgradeable,
            platform: platform,
        };
    }
    else {
        return {
            currentVersion: version,
            state: versionMetadata_1.VersionState.unsupported,
            platform: platform,
        };
    }
}
...
async function generateAppYml(context) {
    ...
    if (((_a = oldProjectSettings.programmingLanguage) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === "csharp") {
        ...
        await context.fsWriteFile(path.join(teamsfx_api_1.SettingsFolderName, Constants.appLocalYmlName), appLocalYmlString);
    }
}
...
async function manifestsMigration(context) {
    ...
    if (!oldAppPackageFolderBackupRes) {
        ...
        throw error_1.MigrationReadFileError(new Error("templates/appPackage does not exist"));
    }
    // Ensure appPackage
    ...
    if (oldManifestExists) {
        ...
        throw error_1.MigrationReadFileError(new Error("templates/appPackage/manifest.template.json does not exist"));
    }
...
async function askUserConfirm(ctx, versionForMigration) {
    ...
    const res = await (globalVars_1.TOOLS === null || globalVars_1.TOOLS === void 0 ? void 0 : globalVars_1.TOOLS.ui.showMessage("warn", v3MigrationUtils_1.migrationNotificationMessage(versionForMigration), true, ...buttons));
