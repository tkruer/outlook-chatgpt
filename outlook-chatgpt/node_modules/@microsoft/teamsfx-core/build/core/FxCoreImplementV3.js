...
class FxCoreV3Implement {
    ...
    async projectVersionCheck(inputs) {
        ...
        if (projectSettingsHelper_1.isValidProjectV3(projectPath) || projectSettingsHelper_1.isValidProjectV2(projectPath)) {
            const currentVersion = await v3MigrationUtils_1.getProjectVersionFromPath(projectPath);
            if (!currentVersion) {
            ...
            const isSupport = v3MigrationUtils_1.getVersionState(currentVersion);
    ...
    async createEnvCopyV3(targetEnvName, sourceEnvName, projectPath) {
        const sourceDotEnvFile = environment_1.environmentManager.getDotEnvPath(sourceEnvName, projectPath);
        const source = await fs_extra_1.default.readFile(sourceDotEnvFile);
        const targetDotEnvFile = environment_1.environmentManager.getDotEnvPath(targetEnvName, projectPath);
...
tslib_1.__decorate([
    hooks_1.hooks([
        errorHandler_1.ErrorHandlerMW,
        questionMW_1.QuestionMW(question_1.getQuestionsForPublishInDeveloperPortal),
        concurrentLocker_1.ConcurrentLockerMW,
        envMW_1.EnvLoaderMW(false),
        contextInjector_1.ContextInjectorMW,
    ]),
